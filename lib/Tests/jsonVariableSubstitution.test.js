...
describe("Test JSON Variable Substitution", () => {
  ...
  before(() => {
    let stub = sinon
      .stub(envVariableUtility_1.EnvTreeUtility, "getEnvVarTree")
      .callsFake(() => {
        let envVariables = new Map([
          ...
          [
            "profile.preimum_level",
            '{"suaggar": "V4", "rok": "V5", "asranja": { "type" : "V6"}}',
          ],
        ]);
        let envVarTree = {
          value: null,
        ...
        for (let [key, value] of envVariables.entries()) {
          if (!envVariableUtility_1.isPredefinedVariable(key)) {
            ...
            for (let variableName of envVariableNameArray) {
              if (
                envVarTreeIterator.child[variableName] === undefined ||
    ...
    let jsonSubsitution =
      new jsonVariableSubstitutionUtility_1.JsonSubstitution();
    isApplied = jsonSubsitution.substituteJsonVariable(
      jsonObject,
      envVariableUtility_1.EnvTreeUtility.getEnvVarTree()
    );
    stub.restore();
  });
  ...
  it("Validate simple string change", () => {
    expect(jsonObject["data"]["ConnectionString"]).to.equal(
      "database_connection"
    );
  ...
  it("Validate inbuilt JSON attributes substitution", () => {
    ...
    expect(jsonObject["constructor"]["name"]).to.equal("newConstructorName");
    expect(jsonObject["constructor"]["valueOf"]).to.equal(
      "constructorNewValue"
    );
  });
  ...
  it("Validate Object", () => {
    expect(jsonObject["profile"]["preimum_level"]).to.deep.equal({
      suaggar: "V4",
      rok: "V5",
      asranja: { type: "V6" },
    });
