...
describe("Test Xml Variable Substitution", () => {
  it("Should substitute", () => {
    ...
    let xmlSubstitution = new xmlVariableSubstitution_1.XmlSubstitution(
    ...
    for (let replacableTokenValue in xmlSubstitution.replacableTokenValues) {
      domContent = domContent
        ...
        .join(xmlSubstitution.replacableTokenValues[replacableTokenValue]);
    }
    ...
    let targetXmlDomUtilityInstance = new xmlDomUtility_1.XmlDomUtility(
      expectedContent
    );
    let expectedXmlDocument = targetXmlDomUtilityInstance.getXmlDom();
    replaceEscapeXMLCharacters(expectedXmlDocument);
    let expectedDomContent =
      "\uFEFF" +
      xmlDomUtilityInstance.getContentWithHeader(expectedXmlDocument);
    expectedDomContent = expectedDomContent
      .split("APOS_CHARACTER_TOKEN")
      .join("'");
