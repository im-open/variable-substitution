...
describe("Test variable substitution main", () => {
  ...
  before(() => {
    ...
    JsonSubstitutionMock = sinon.mock(
      jsonVariableSubstitutionUtility_1.JsonSubstitution
    );
  ...
  it("Valid XML", () => {
    ...
    chai_1.expect(
      spy.calledWith("Applying variable substitution on XML file: " + file)
    ).to.be.true;
  });
  it("Valid JSON", () => {
    ...
    chai_1.expect(
      spy.calledWith("Applying variable substitution on JSON file: " + file)
    ).to.be.true;
  });
  it("Invalid JSON", () => {
  ...
  it("Valid YAML", () => {
    ...
    chai_1.expect(
      spy.calledWith("Applying variable substitution on YAML file: " + file)
    ).to.be.true;
  });
  it("Invalid YAML", () => {
